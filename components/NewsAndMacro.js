        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-sm font-medium">
                  {macroData.psxIndex.toLocaleString()}
                </div>
                <div
                  className={`text-xs ${
                    macroData.psxChange >= 0 ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {macroData.psxChange >= 0 ? "+" : ""}
                  {macroData.psxChange}%
                </div>
              </div>
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="relative w-32 h-32 mx-auto">
            <svg className="w-32 h-32 transform -rotate-90" viewBox="0 0 32 32">
              ...
              <circle
                ...
                strokeWidth="4"
                strokeDasharray={`${
                  (analystRatings.buy / analystRatings.total) * 88
                } 88`}
                strokeDashoffset="0"
              />
              {/* Hold segment */}
              <circle
                ...
                strokeWidth="4"
                strokeDasharray={`${
                  (analystRatings.hold / analystRatings.total) * 88
                } 88`}
                strokeDashoffset={`-${
                  (analystRatings.buy / analystRatings.total) * 88
                }`}
              ...
              <circle
                ...
                strokeWidth="4"
                strokeDasharray={`${
                  (analystRatings.sell / analystRatings.total) * 88
                } 88`}
                strokeDashoffset={`-${
                  ((analystRatings.buy + analystRatings.hold) /
                    analystRatings.total) *
                  88
                }`}
              />
            </svg>
          ...
          <div className="pt-3 border-t text-center">
            ...
            <div className="text-lg font-medium">
              PKR {analystRatings.avgTarget}
            </div>
            <div className="text-sm text-green-600">
              +
              {(
                ((analystRatings.avgTarget - stock.currentPrice) /
                  stock.currentPrice) *
                100
              ).toFixed(1)}
              % upside
